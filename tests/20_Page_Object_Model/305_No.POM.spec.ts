...
test.describe('Data Driven Test Simple', () => {
    const data = {
    ...
    test(`Login with : ${data.description}`, async ({ page }) => {
        ...
        let textboxEmail = page.getByRole("textbox", { name: "Email Address" });
        let textboxPassword = page.getByRole("textbox", { name: "Password" }).or(page.locator("#password")).or(page.locator("[name=\"password\"]"));
        let buttonLogin = page.getByRole("button", { name: "Login to Practice Account" }).or(page.getByTestId("login-button")).or(page.getByText("Login to Practice Account"));

        await textboxEmail.fill(data.username);
        await textboxPassword.fill(data.password);
